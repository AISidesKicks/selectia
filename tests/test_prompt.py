...
@needs_tok
def test_label_table_capacity_is_tokenizer_driven_and_starts_a_to_j():
    names, ids, _ = prompt.label_table(TOK)
    cap = prompt.label_capacity(TOK)
    assert len(names) == len(ids) == cap
    assert prompt.WIDE_MIN <= cap <= prompt.MAX_OPTIONS
    assert names[:10] == list("ABCDEFGHIJ") and len(set(ids)) == cap
...
def test_wide_rendering_uses_one_label_token_per_option():
    cap = prompt.label_capacity(TOK)
    n = max(prompt.WIDE_MIN, min(cap, 60))
    gold = n - 1
    opts = [f"label_{i}" for i in range(n)]
    ex = Ex("some text", [Q("Which label?", opts, gold)])
    b = prompt.build(ex, TOK, max_options=cap)
    names, ids, _ = prompt.label_table(TOK)
    text = TOK.decode(b["ids"])
    assert list(b["nopts"]) == [n] and f"({names[n - 1]}) {opts[b['perms'][0][n - 1]]}" in text
    narrow = prompt.build(ex, TOK, rng=random.Random(0), max_options=10)
    assert list(narrow["nopts"]) == [10] and opts[gold] in TOK.decode(narrow["ids"])   # sub-sampling keeps the gold option
    assert narrow["perms"][0][narrow["golds"][0]] == gold


@needs_tok
def test_wide_rendering_refuses_more_options_than_the_tokenizer_supplies():
    cap = prompt.label_capacity(TOK)
    opts = [f"o{i}" for i in range(cap + 1)]
    ex = Ex("some text", [Q("Which?", opts, 0)])
    with pytest.raises(AssertionError):
        prompt.build(ex, TOK, max_options=cap + 1)
