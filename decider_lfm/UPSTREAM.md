...
`decider_lfm/` is a port of [Mapika/decider](https://github.com/Mapika/decider) at commit
`c4daaac28af9fea95d627015cffa2dd5a5926ee6`, licensed Apache-2.0 (see `CODE-LICENSE` and `NOTICE`).
The Qwen3.5 delta-net engine is intentionally not vendored yet.

## What changed in the port

- **Package name**: every `decider.*` import became `decider_lfm.*`.
...
- **`train.py`**: adds `--grad_ckpt`, `--device`, `--yesmom`, `--isolated_levels`, `--temperature`, `--version`;
  `--max_options 0` means "auto = tokenizer capacity". It writes `decider_config.json` into the saved model
  folder, which upstream never did but `Decider` requires at runtime.
- **`infer.py`**: option validation uses `label_capacity`, and `decider_config.json`'s `yesmom` flag restricts
