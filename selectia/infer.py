...
class Selectia:
    ...
    def decide_batch(self, requests, max_ctx_tokens=1536):
        ...
        items = [build(e, self.m.tok, _NoShuffle(), max_options=self.max_options, max_ctx_tokens=max_ctx_tokens) for e in exs]
    ...
    def system_one(self, state, questions, independent=True, max_state_tokens=32768, max_fwd_tokens=65536, layout=None, isolated=None):
        ...
        class _Keep:
            def shuffle(self, x): pass
            def sample(self, xs, k): return xs[:k]
        items = [build(Example(ctx, [Q(r["question"], opts(r), 0) for r in row]), self.m.tok, _Keep(), max_options=self.max_options,
    ...
    def decide_json_batch(self, requests, **kw):
        ...
        if self.yesmom:
            for _, schema in requests:
                bad = [k for k, s in schema.items() if s.get("type", "choice") != "bool"]
                if bad:
                    raise ValueError(f"yesmom model: only bool questions are supported, got {bad}")
