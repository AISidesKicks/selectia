...
MAX_OPTIONS = 255          # upper bound on the label head. > NARROW options: "wide" rendering, one label token per option:
                           # A..Z then two-letter upper-case strings that are single tokens (AA, AB, ...).  The real capacity
                           # is tokenizer dependent (label_capacity); MAX_OPTIONS is only a ceiling for --max_options.
WIDE_MIN = 11              # a wide rendering needs at least one label past A..J
...
def _tok_key(tok):
    """Cache key that is stable per tokenizer identity: id(tok) alone can be reused after a tokenizer is
    garbage collected, which would silently serve one model's label table to another size."""
    return (getattr(tok, "name_or_path", None), len(tok))


def _enc_opt(tok, text):
    """Token ids of ") <option text>" (cached: fixed label sets repeat the same strings millions of times)."""
    key = (_tok_key(tok), text)
...
def label_table(tok):
    """(label strings, label token ids, "\\n(" ids).  One single-token label per option: A..Z then the first
    two-letter upper-case strings the tokenizer maps to a single token.  The width is tokenizer dependent
    (the 128k-vocab LFM2.5-2.6B yields the full MAX_OPTIONS, the 65536-vocab sizes fewer): the returned
    length is the capacity, not the constant.  The first NARROW labels are A..J so narrow questions are unchanged."""
    key = _tok_key(tok)
    if key not in _LABELS:
        ...
        for n in names:
            ...
            if len(out) == MAX_OPTIONS:
                break
        assert len(out) >= WIDE_MIN, f"tokenizer supplies only {len(out)} single-token option labels"
        assert len({i for _, i in out}) == len(out), "duplicate label token ids"
        _LABELS[key] = ([n for n, _ in out], [i for _, i in out],
                        tok.encode("\n(", add_special_tokens=False))
    return _LABELS[key]


def label_capacity(tok):
    """How many single-token option labels this tokenizer can supply (<= MAX_OPTIONS).  Fixes --max_options."""
    return len(label_table(tok)[1])
...
def _options_ids(tok, q, opts):
    ...
    assert len(opts) <= len(lab_ids), f"wide rendering needs {len(opts)} single-token labels, tokenizer supplies {len(lab_ids)}"
    out = []
